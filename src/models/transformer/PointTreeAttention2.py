...
class PTA(nn.Module):
    ...
    def process_coarse_level(self, query, key, value, topk, key_padding_mask, query_padding_mask):
        B, M, H, C = query.shape    # 1 605 8 32
        B, N, H, C = key.shape
        query = F.elu(query) + 1.
        key = F.elu(key) + 1.
#维度错误
        # kv = torch.einsum('bhnd,bhne->bhde', key, value)  # [B, H, C, C]
        # k_sum = key.sum(dim=2)  # [B, H, C]
        # z = 1.0 / (torch.einsum('bhnd,bhd->bhn', query, k_sum) + 1e-6)  # [B, H, M]
        # message = torch.einsum('bhde,bhnd,bhn->bhne', kv, query, z)  # [B, H, M, C]
        # message = message.reshape(B, M, H * C)
        #
        # qk_approx = torch.einsum('bhnd,bhnd->bhn', query, key.mean(dim=2, keepdim=True))  # [B, H, M]
        # A_mean = qk_approx.mean(dim=1)  # [B, M]
        # _, topk_idx = torch.topk(A_mean, dim=-1, k=topk, largest=True)  # [B, topk]
        # message = self.proj(message)
        # message = self.proj_drop(message)
        # value_flat = value.reshape(B, N, H * C)
        # message = value_flat + message  # 残差连接


#维度正确
        kv = torch.einsum('bmhd,bmhe->bhde', key, value)  # [B, H, C, C]

        # 2. 计算归一化因子
        k_sum = key.sum(dim=1)  # [B, H, C]
        z = 1.0 / (torch.einsum('bmhd,bhd->bmh', query, k_sum) + 1e-6)  # [B, M, H]

        # 3. 聚合消息（维度对齐）
        message = torch.einsum('bhde,bmhd,bmh->bmhe', kv, query, z)  # [B, M, H, C]
        # ================================================
        message = message.reshape(B, M, -1)

        # =============== PTT层级选择机制 ===============
        # 1. 近似注意力权重计算
        qk_approx = torch.einsum('bmhd,bnhd->bmnh', query, key)  # [B, M, N, H]

        # 2. 选择top-k关键区域
        A_mean = qk_approx.mean(dim=3)  # [B, M, N]  # 关键修复：在头维度平均
        _, topk_idx = torch.topk(A_mean, dim=2, k=topk, largest=True)  # [B, M, topk]  #在QK计算的过程不一样

        # 3. 残差连接 (保持原始PTT设计)
        message = self.proj(message)
        message = self.proj_drop(message)
        query_flat = query.reshape(B, M, -1)  # 修复：使用查询特征而非值特征
        message = query_flat + message  # 残差连接
...
#原
        ...
        if ONE_BATCH == True:
            ...
            topk_idx = topk_idx[~query_padding_mask, :]  #正常  605 8

        return None, message, topk_idx
...
#维度正确
        ...
        qk_approx = torch.einsum("bmhc,bnhc->bmnh", query, key)  # [B, M, N, H]
        A_mean = qk_approx.mean(dim=-1)  # [B, M, N]

        # 在 N 维度上取 top-k
        _, topk_idx = torch.topk(A_mean, dim=-1, k=topk, largest=True)  # [B, M, topk]】

        # 只保留 topk 的 query
        message_sparse = torch.zeros_like(message)
        for b in range(B):
            message_sparse[b, topk_idx[b]] = message[b, topk_idx[b]]

        # === 5. 残差增强 (保持 PTT 原版思想: value + message)
        v_flat = value.reshape(B,M, H * C).mean(dim=1, keepdim=True)  # [B,1,H*C] 全局残差
        message = message_sparse + v_flat.expand_as(message_sparse)
