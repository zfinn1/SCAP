...
class GenericRegModel(GenericModel, ABC):
    ...
    def test_step(self, batch, batch_idx):
        ...
        pred = self.forward(batch)
        losses = self.compute_loss(pred, batch,batch_idx)
        metrics = self._compute_metrics(pred, batch)
    ...
    def _aggregate_metrics(self, metrics):
        ...
        for p in range(num_pred):
            ...
            for rk in rot_err_keys:
                ...
                # if suffix == 'final':
                #     rot_thresholds = torch.arange(0.0, 0.5, 0.0005)  # [0, 0.8, ..., 20.0]
                #     for thr in rot_thresholds:
                #         reg_success_dynamic = torch.logical_and(
                #             metrics_cat[rk][p, :] < self.reg_success_thresh_rot,
                #             metrics_cat[tk][p, :] < thr
                #         )
                #         xxx = reg_success_dynamic.float().mean()
                #         print("=========================================")
                #         print(thr, xxx)
                #         print("=========================================")

                    # 箱式图代码：
                save_dir = "error_results"
                os.makedirs(save_dir, exist_ok=True)

                trans_errors = avg_metrics['trans_err_0_hist'].cpu().numpy()

                # 提取所有旋转误差值（需要对应的键）
                # 假设旋转误差键为 'rot_err_final_hist'
                rot_errors = avg_metrics['rot_err_0_hist'].cpu().numpy()

                # 保存平移误差到txt
                np.savetxt(os.path.join(save_dir, "translation_errors.txt"),
                           trans_errors,
                           fmt='%.8f',
                           header="Translation Errors")

                # 保存旋转误差到txt
                np.savetxt(os.path.join(save_dir, "rotation_errors.txt"),
                           rot_errors,
                           fmt='%.8f',
                           header="Rotation Errors (degrees)")

                # 保存合并的平移和旋转误差
                combined_errors = np.column_stack((rot_errors, trans_errors))
                np.savetxt(os.path.join(save_dir, "combined_errors.txt"),
                           combined_errors,
                           fmt='%.8f %.8f',
                           header="Rotation(deg) Translation")

                print(f"保存了 {len(trans_errors)} 个样本的误差数据到 {save_dir}/")
