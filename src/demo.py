...
def visualize_result(src_xyz: np.ndarray, tgt_xyz: np.ndarray,
    ...
    src_pcd.colors = o3d.utility.Vector3dVector(np.tile([1, 0, 0], (src_xyz.shape[0], 1)))  # 红色
    o3d.io.write_point_cloud(f"example_{example_idx}_src_original.ply", src_pcd)

    # 2. 原始目标点云（绿色）
    tgt_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(tgt_xyz))
    tgt_pcd.colors = o3d.utility.Vector3dVector(np.tile([0, 1, 0], (tgt_xyz.shape[0], 1)))  # 绿色
    o3d.io.write_point_cloud(f"example_{example_idx}_tgt_original.ply", tgt_pcd)

    # 3. 过滤后的源关键点（彩色，基于重叠概率）
    kp_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(src_kp[m]))
    kp_pcd.colors = o3d.utility.Vector3dVector(overlap_colors[m] / 255.0)  # 归一化到0-1
    o3d.io.write_point_cloud(f"example_{example_idx}_src_keypoints.ply", kp_pcd)

    # 4. 过滤后的目标对应关键点（与源关键点同色）
    corr_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(src2tgt[m]))
    corr_pcd.colors = o3d.utility.Vector3dVector(overlap_colors[m] / 255.0)
    o3d.io.write_point_cloud(f"example_{example_idx}_tgt_correspondences.ply", corr_pcd)

    # 5. 保存配准后的源点云（红色）
    src_registered = se3_transform(pose, src_xyz)  # 应用位姿变换
    reg_src_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(src_registered))
    reg_src_pcd.colors = o3d.utility.Vector3dVector(np.tile([1, 0, 0], (src_registered.shape[0], 1)))  # 红色
    o3d.io.write_point_cloud(f"example_{example_idx}_src_registered.ply", reg_src_pcd)

    # ==================== 新增：保存配准后的目标点云（绿色） ====================
    # 目标点云无需变换（配准是将源点云变换到目标坐标系）
    reg_tgt_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(tgt_xyz))  # 目标点云坐标不变
    reg_tgt_pcd.colors = o3d.utility.Vector3dVector(np.tile([0, 1, 0], (tgt_xyz.shape[0], 1)))  # 绿色
    o3d.io.write_point_cloud(f"example_{example_idx}_tgt_registered.ply", reg_tgt_pcd)

    print(f"PLY files saved for example {example_idx} (6 files in total)")
...
def main():
    ...
    visualize_result(
        src_xyz, tgt_xyz, src_kp, src2tgt, overlap_score, pose,
        threshold=opt.threshold,
        example_idx=2  # 新增：传入示例编号
    )
