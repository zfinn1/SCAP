...
class ModelNetHdf(Dataset):
    ...
    def __getitem__(self, item):
        ...
        src_pts=torch.from_numpy(sample['points_src'][:, :3])
        tgt_pts=torch.from_numpy(sample['points_ref'][:, :3])
        base_dir = "../data/modelnet_demo_data"
        os.makedirs(base_dir, exist_ok=True)

        src_filename = os.path.join(base_dir, f"modelnet_test_{item}_0.ply")
        tgt_filename = os.path.join(base_dir, f"modelnet_test_{item}_1.ply")

        self.save_ply(src_pts.numpy(), src_filename)
        self.save_ply(tgt_pts.numpy(), tgt_filename)
